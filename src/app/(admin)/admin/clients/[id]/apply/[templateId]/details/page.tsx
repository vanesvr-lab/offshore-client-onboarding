...
const EMPTY_FORM = {
  business_name: "", business_type: "", business_country: "", business_address: "",
  contact_name: "", contact_email: "", contact_phone: "", contact_title: "",
};

export default function AdminWizardDetailsPage({
  params,
}: {
  ...
  const [applicationId, setApplicationId] = useState<string | null>(existingAppId);
  ...
  async function saveProgress(andContinue = false) {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/admin/applications/upsert", {
        method: "POST",
      ...
      if (andContinue) {
        router.push(`/admin/clients/${params.id}/apply/${params.templateId}/documents?applicationId=${appId}`);
      } else {
